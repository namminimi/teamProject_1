...
const homeRun = document.querySelector("#attack input:nth-child(3)")
// const skill_batter = document.querySelector("#attack input:nth-child(4)")
// 수비 버튼연결
...
const computerchar = document.querySelector("#character2");
const userchar = document.querySelector("#character");
const wrap =document.querySelector("#wrap");



function pitcher() {
    if(character.style.left == `25%` &&  character.style.top == `35%` && user.ball == 4){
        character.style.left = `50%`
        character.style.top = `80%`
        user.ball = 0;    
    }
    if(character.style.left == `50%` &&  character.style.top == `10%` && user.ball == 4){
        character.style.left = `25%`
        character.style.top = `35%`
        user.ball = 0;    
    }
    if(character.style.left == `73%` &&  character.style.top == `32%` && user.ball == 4){
        character.style.left = `50%`
        character.style.top = `10%`
        user.ball = 0;    
    }
    if (user.ball == 4) {
        console.log(1)
        character.style.left = `73%`
        character.style.top = `32%`
        user.ball = 0;
    }   
}
// --------------------------공격측 이동코드
function attack() {
    if(character2.style.left == `25%` &&  character2.style.top == `35%` && computer.ball == 4){
        character2.style.left = `50%`
        character2.style.top = `80%`
        computer.ball = 0;    
    }
    if(character2.style.left == `50%` &&  character2.style.top == `10%` && computer.ball == 4){
        character2.style.left = `25%`
        character2.style.top = `35%`
        computer.ball = 0;    
    }
    if(character2.style.left == `73%` &&  character2.style.top == `32%` && computer.ball == 4){
        character2.style.left = `50%`
        character2.style.top = `10%`
        computer.ball = 0;    
    }
    if (computer.ball == 4) {
        console.log(1)
        character2.style.left = `73%`
        character2.style.top = `32%`
        computer.ball = 0;
    }   
}
...
// 왜이렇게잘하지...ball.entries.apply.apply.